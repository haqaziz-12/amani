...
const emptyProduct = {
  name: "", slug: "", size: "", quality: "", materials: "", description: "",
  washing_type: "", collection: "", price_note: "Price on Enquiry",
...
export default function AdminPage() {
  ...
  const loadProducts = async () => {
    ...
    if (error) {
      ...
      if (!data || data.length === 0) setProductsError("No products found. Check Table Editor.");
    }
  ...
  const uploadFile = async (
    ...
    try {
      ...
      if (existing?.id) {
        await supabase.from("site_settings").update({ [column]: publicUrl, updated_at: new Date().toISOString() } as any).eq("id", existing.id);
      } else {
        await supabase.from("site_settings").insert({ [column]: publicUrl } as any);
      }
      setUrl(publicUrl);
  ...
  const deleteProduct = async (id: string) => {
    ...
    else { setProductMessage("Product deleted"); loadProducts(); setTimeout(() => setProductMessage(""), 3000); }
  };
  ...
  return (
    <div className="min-h-screen bg-muted/30">
      ...
      <div className="container-wide py-8 px-4">
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between flex-wrap gap-3">
              ...
              <div className="flex gap-2">
                <button onClick={loadProducts} className="bg-white border border-border px-4 py-2 rounded-lg text-sm font-medium hover:bg-muted">Refresh</button>
            ...
            {productsError && <div className="px-4 py-3 rounded-lg text-sm bg-amber-50 text-amber-800 border border-amber-200">{productsError}</div>}
            {productMessage && <div className={`px-4 py-3 rounded-lg text-sm ${productMessage.startsWith("Error") ? "bg-red-50 text-red-700" : "bg-green-50 text-green-700"}`}>{productMessage}</div>}

            {editingProduct && (
              <div className="bg-white rounded-2xl border border-border p-6 shadow-sm space-y-4">
                <h3 className="font-semibold text-brand-dark">{editingProduct.id ? "Edit Product" : "New Product"}</h3>
                <div className="grid sm:grid-cols-2 gap-4">
                  <div><label className="block text-sm font-medium mb-1">Name *</label>
                    <input value={editingProduct.name} onChange={(e) => setEditingProduct({ ...editingProduct, name: e.target.value })}
                      className="w-full px-3 py-2 rounded-lg border border-border" /></div>
                  <div><label className="block text-sm font-medium mb-1">Slug</label>
                    <input value={editingProduct.slug} onChange={(e) => setEditingProduct({ ...editingProduct, slug: e.target.value })}
                      className="w-full px-3 py-2 rounded-lg border border-border" placeholder="auto from name" /></div>
                ...
                <div><label className="block text-sm font-medium mb-1">Description</label>
                  <textarea value={editingProduct.description} onChange={(e) => setEditingProduct({ ...editingProduct, description: e.target.value })}
                    rows={4} className="w-full px-3 py-2 rounded-lg border border-border" /></div>
                <div className="grid sm:grid-cols-3 gap-4">
                  {(["image_front", "image_back", "image_detail"] as const).map((field) => (
                    <div key={field}>
                      ...
                      {editingProduct[field] && <Image src={editingProduct[field]} alt="" width={120} height={90} className="rounded mb-2 object-cover" unoptimized />}
                      <input type="file" accept="image/*" onChange={(e) => { const f = e.target.files?.[0]; if (f) uploadProductImage(f, field); }}
                        className="text-sm" disabled={uploadingImage === field} />
                      {uploadingImage === field && <Loader2 className="w-4 h-4 animate-spin mt-1" />}
                    </div>
                  ))}
                </div>
                <div className="flex gap-2">
                  <button onClick={saveProduct} disabled={savingProduct}
                    ...
                    {savingProduct ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />} Save
                  </button>
                  <button onClick={() => setEditingProduct(null)} className="px-4 py-2 rounded-lg text-sm border border-border">Cancel</button>
                </div>
              </div>
            )}
            ...
            {productsLoading ? (
              ...
              <p className="text-brand-muted text-center py-8">No products in database yet.</p>
            ) : (
        ...
        {(activeTab === "about" || activeTab === "services" || activeTab === "craftsmanship") && (
          <ContentEditor tab={activeTab} />
        )}
